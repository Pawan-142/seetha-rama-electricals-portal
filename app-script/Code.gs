  ...
  *    manually add: https://mail.google.com/   * 6. Copy the Web App URL and paste it into js/config.js.
  ...
  var SPREADSHEET_ID = '11xzihA8eclDMrW4NxvQtlR5b7z6gC6zfZ1S0HSfgjIw';
  var ssCache = null;

  function getSpreadsheet() {
    if (!ssCache) {
      ssCache = SpreadsheetApp.openById(SPREADSHEET_ID);
    }
    return ssCache;
  }
  ...
  function doGet(e) {
    ...
    try {
      if (action === 'initialize') {
        initializeDatabase();
        response = { success: true, message: 'Database initialized/migrated successfully.' };
  ...
  function handleAddProduct(data) {
    ...
    // Construct row matching headers dynamically (casing-insensitive to support capitalization in Google Sheets)!
  ...
  function handleAddCustomer(data) {
    ...
    // Loose matching for headers to support different column names and order
    var idIndex = -1, nameIndex = -1, phoneIndex = -1, emailIndex = -1, addressIndex = -1;
    for (var i = 0; i < headers.length; i++) {
      var h = headers[i].toString().trim().toLowerCase();
      if (h === 'customerid' || h === 'id' || h.indexOf('id') !== -1) idIndex = i;
      else if (h === 'name' || h === 'customer name' || h.indexOf('name') !== -1) nameIndex = i;
      else if (h === 'phone' || h === 'mobile' || h.indexOf('phone') !== -1 || h.indexOf('number') !== -1 || h.indexOf('mobile') !== -1) phoneIndex = i;
      else if (h === 'email' || h === 'mail' || h.indexOf('email') !== -1 || h.indexOf('mail') !== -1) emailIndex = i;
      else if (h === 'address' || h.indexOf('address') !== -1) addressIndex = i;
    }
    ...
    for (var r = 1; r < rows.length; r++) {
      ...
      var existingName = nameIndex !== -1 ? rows[r][nameIndex].toString().trim() : 'Unknown Customer';
      
      if (newPhone !== "" && rowPhone === newPhone) {
        return { 
          success: false, 
          message: 'Customer account already exists with this phone number (' + newPhone + ') registered under Name: "' + existingName + '".' 
        };
      }
      if (newEmail !== "" && rowEmail === newEmail) {
        return { 
          success: false, 
          message: 'Customer account already exists with this email (' + newEmail + ') registered under Name: "' + existingName + '".' 
        };
      }
    }
    ...
    // Construct row matching headers dynamically based on matched indexes
    var newRow = new Array(headers.length);
    for (var i = 0; i < headers.length; i++) {
      newRow[i] = '';
    }
    if (idIndex !== -1) newRow[idIndex] = customerId;
    if (nameIndex !== -1) newRow[nameIndex] = data.name || '';
    if (phoneIndex !== -1) newRow[phoneIndex] = data.phone || '';
    if (emailIndex !== -1) newRow[emailIndex] = data.email || '';
    if (addressIndex !== -1) newRow[addressIndex] = data.address || '';
  ...
  function handleSaveInvoice(data) {
    ...
    // 2. Deduct inventory quantities (batch optimize stock writes)
    ...
    if (idIndex !== -1 && stockIndex !== -1) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        for (var r = 1; r < invRows.length; r++) {
          if (invRows[r][idIndex].toString() === pId.toString()) {
            ...
            invRows[r][stockIndex] = currentStock - qtyToDeduct;
            hasUpdates = true;
            break;
          }
        }
      ...
      if (hasUpdates) {
        var stockValues = [];
        for (var r = 1; r < invRows.length; r++) {
          stockValues.push([invRows[r][stockIndex]]);
        }
        inventorySheet.getRange(2, stockIndex + 1, stockValues.length, 1).setValues(stockValues);
      }
    }
  ...
  function handleEditCustomer(data) {
    ...
    // Loose matching for headers to support different column names and order
    var idIndex = -1, nameIndex = -1, phoneIndex = -1, emailIndex = -1, addrIndex = -1;
    for (var i = 0; i < headers.length; i++) {
      var h = headers[i].toString().trim().toLowerCase();
      if (h === 'customerid' || h === 'id' || h.indexOf('id') !== -1) idIndex = i;
      else if (h === 'name' || h === 'customer name' || h.indexOf('name') !== -1) nameIndex = i;
      ...
      else if (h === 'address' || h.indexOf('address') !== -1) addrIndex = i;
    }
    ...
    for (var r = 1; r < rows.length; r++) {
      if (idIndex !== -1 && rows[r][idIndex].toString() === cId.toString()) {
        foundRow = r + 1;
        break;
      }
    }
    ...
    for (var r = 1; r < rows.length; r++) {
      if ((r + 1) !== foundRow) {
        ...
        if (newPhone !== "" && rowPhone === newPhone) {
          return { 
            success: false, 
            message: 'Another customer account already exists with this phone number (' + newPhone + ') registered under Name: "' + existingName + '".' 
          };
        }
        if (newEmail !== "" && rowEmail === newEmail) {
          return { 
            success: false, 
            message: 'Another customer account already exists with this email (' + newEmail + ') registered under Name: "' + existingName + '".' 
          };
        }
      }
    }
